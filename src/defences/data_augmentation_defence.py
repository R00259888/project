...
def __generate_negative_sequence(model, length):
    ...
    mouse_events = [MouseEvent(timestamp=0, x=0, y=0, button_pressed=0, subject_id=-1)]
    for i in range(deltas_len):
        mouse_events.append(MouseEvent(
            timestamp=i + 1,
            x=int(round(mouse_events[-1].x - deltas[i, 0])),
            y=int(round(mouse_events[-1].y - deltas[i, 1])),
            button_pressed=0,
            subject_id=-1,
        ))
    data = np.array([[mouse_event.timestamp, mouse_event.x, mouse_event.y, mouse_event.button_pressed, mouse_event.subject_id] for mouse_event in mouse_events])
...
def data_augmentation_defence(dataset, subject_id):
    ...
    for mouse_event_sequence in subject_sequences:
        ...
        mouse_event_sequence_length = len(augmented_mouse_event_sequence)
        ...
        for index in random.sample(range(mouse_event_sequence_length), min(mutation_count, mouse_event_sequence_length)):
            augmented_mouse_event_sequence.data[index, 1] += int(np.random.normal(0, 5))
            augmented_mouse_event_sequence.data[index, 2] += int(np.random.normal(0, 5))
